...
const char *topicLightBulbState =
    "livingroom/lr-NodeMCU/Lightbulb-state/relay-lb-1";

const char *topicHeat = "livingroom/lr-NodeMCU/Heat/relay-ht-1";
const char *topicHeatState = "livingroom/lr-NodeMCU/Heat-state/relay-ht-1";
...
void publishRelayState() {
  ...
  if (digitalRead(BULB_DPIN) == HIGH) {
    ...
    Serial.println("Lightbulb is ON");

  } else {
    client.publish(topicLightBulbState, "OFF");
    Serial.println("Lightbulb is OFF");
  }

  if (digitalRead(HEAT_PIN) == HIGH) {
    client.publish(topicHeatState, "ON");
    Serial.println("Heat is ON");

  } else {
    client.publish(topicHeatState, "OFF");
    Serial.println("Heat is OFF");
  }
}

void relayHandler(char *command, char *topic) {
  if (strcmp(topic, topicHeat) == 0) {
  ...
  } else if (strcmp(topic, topicLightBulb) == 0) {
    if (strcmp(command, "ON") == 0) {
...
void setup(void) {
  ...
  JsonObject lightBulbSensor = sensors.createNestedObject();
  lightBulbSensor["type"] = "Lightbulb";
  lightBulbSensor["name"] = "relay-lb-1";

  JsonObject heatSensor = sensors.createNestedObject();
  heatSensor["type"] = "Heat";
  heatSensor["name"] = "relay-ht-1";
